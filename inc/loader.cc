...
static bfd* open_bfd(string &fname){
    ...
    /*Some versions of bfd_check_format pessimistically set a wrong_format
     error before detecting the format and then neglect the unset it once
     the format has been detected.we unset it manually to prevent problems.
     */
    bfd_set_error(bfd_error_no_error);
    if(bfd_get_flavour(bfd_h) == bfd_target_unknown_flavour){
        fprintf(stderr,"unrecongized format for binary '%s' (%s)\n",fname.c_str(),bfd_errmsg(bfd_get_error()));
        return NULL;
    }
        return bfd_h;
}
...
static int load_binary_bfd(string &fname,Binary *bin,Binary::BinaryType type){
    ...
    switch(bfd_info->mach){
        ...
        case bfd_mach_x86_64:
            ...
            break;
        default:
            fprintf(stderr,"unsupported architeture (%s)\n",bfd_info->printable_name);
            goto fail;
    }
...
static int load_symbols_bfd(bfd *bfd_h,Binary *bin){
    ...
    if(n < 0){
        fprintf(stderr,"failed to read the symtab (%s)\n",bfd_errmsg(bfd_get_error()));
        goto fail;
    }else if(n){
...
static int load_dynsym_bfd(bfd *bfd_h,Binary *bin){
    ...
    if(n < 0){
        ...
        if(nsyms < 0){
            fprintf(stderr,"failed to read dynamic symbol symtab(%s)\n",bfd_errmsg(bfd_get_error()));
            goto fail;
        }
        for(i=0;i<nsyms;i++){
